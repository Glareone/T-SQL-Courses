...
--SET @DayCount = DATEDIFF(day,@StartFiscalYear,@EndFiscalYear);
--SELECT @DayCount;
...
BEGIN
	...
	SET @FiscalMonthEndFlag = 
								CASE 
									...
									OR	(@FiscalDayInYearNumber = 371 AND DATEDIFF(DAY,@StartFiscalYear, @EndFiscalYear) > 364)
									THEN 1
									ELSE 0
								END
	...
	SET @FiscalMonthEndDate = 
								CASE 
									...
									WHEN	(@FiscalDayInYearNumber <= 371 AND @FiscalDayInYearNumber > 336 AND (DATEDIFF(DAY,@StartFiscalYear,@EndFiscalYear) > 364)) THEN DATEADD(DAY,370,@StartFiscalYear) 
								END
