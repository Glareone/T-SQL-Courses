...
BEGIN
    ...
    CalendarDate DATETIME NOT NULL UNIQUE, 
...
END
GO
	PRINT 'FUNCTION FindFiscalStartYear Added';
--------------------------------------------
-- 2) Create function which find fiscal year end date and returns it.
IF EXISTS ( SELECT  1
					FROM    Information_schema.Routines
					WHERE   Specific_schema = 'dbo'
                    AND specific_name = 'FindFiscalEndYearByStartFiscalYear'
                    AND Routine_Type = 'FUNCTION' ) 
	DROP FUNCTION dbo.FindFiscalEndYearByStartFiscalYear
GO        
   
CREATE FUNCTION dbo.FindFiscalEndYearByStartFiscalYear(@StartFiscalYear DATE)
RETURNS DATE
AS
BEGIN
	DECLARE 
	...
	SET @NextYear = CAST('01-01-' + CONVERT(VARCHAR, DATEPART(year,DATEADD(year,1,@StartFiscalYear)))AS DATE); --выбираем 1 января следующего года
...
END		
GO
	PRINT 'FUNCTION FindFiscalEndYear Added';